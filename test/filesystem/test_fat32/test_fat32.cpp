 ...
 * @brief   Unit tests for testing the FAT32 interface.
 ==============================================================================
 */
...
{
    /* Setup the block buffer */
    std::array<uint8_t, 512> buffer{0};
...
void test_partition()
{
    ...
    const uint32_t FAT_Begin = 0x01 + 0x01;
    const uint32_t FAT_Size = 0x32;
    const uint32_t First_Data_Sector = FAT_Begin + (2*FAT_Size) + fat32::BPB::root_directory_sectors(buffer.begin());
    const uint32_t Root_Directory_Cluster = 0x40;
    const uint32_t Sectors_per_Cluster = 0x40;
